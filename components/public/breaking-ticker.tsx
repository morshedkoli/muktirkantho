...
type TickerItem = { id: string; title: string; slug: string };
...
/**
 * The ticker sits in the shared public layout, so without a cache it costs one
 * query on every request to every dynamic page (category, tag, district,
 * search). Cached for the same 60s window the article pages revalidate on, and
 * tagged so publishing a post refreshes it immediately.
 */
const getBreakingItems = unstable_cache(
  async (): Promise<TickerItem[]> => {
    try {
      ...
      // render nothing if the DB is unavailable
      return [];
    }
  },
  ["breaking-ticker"],
...
export async function BreakingTickerServer() {
  return <BreakingTicker items={await getBreakingItems()} />;
}
...
export function BreakingTicker({ items }: BreakingTickerProps) {
  ...
  return (
    <div className="bg-[var(--np-breaking-bar)] overflow-hidden">
      <div className="mx-auto max-w-7xl">
        <div className="flex items-stretch min-h-[40px]">

          {/* "ব্রেকিং" badge */}
          <div className="flex items-center gap-2 bg-[var(--np-breaking-badge)] px-3 sm:px-5 shrink-0 border-r border-[var(--np-breaking-edge)]">
            {/* Pulsing dot */}
          ...
          <div className="flex-1 overflow-hidden relative">
            ...
            <div className="pointer-events-none absolute right-0 top-0 h-full w-12 bg-gradient-to-l from-[var(--np-breaking-bar)] to-transparent z-10" />
