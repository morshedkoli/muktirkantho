...
export function LocationFilter({ divisions }: LocationFilterProps) {
  ...
  return (
    <div className="rounded-xl border border-[var(--np-border)] bg-[var(--np-card)] p-5 shadow-[var(--np-shadow)]">
      {/* "এলাকা" rather than "বিভাগ" in the heading: the categories panel
          directly below already uses বিভাগসমূহ, and repeating the word for two
          unrelated things in one rail reads as a mistake. */}
      <h3 className="np-headline-sm text-[var(--np-text-primary)]">এলাকা অনুযায়ী খবর</h3>
      <p className="mt-1 text-xs text-[var(--np-text-secondary)]">
        বিভাগ, জেলা ও উপজেলা অনুযায়ী সংবাদ দেখুন।
      </p>
